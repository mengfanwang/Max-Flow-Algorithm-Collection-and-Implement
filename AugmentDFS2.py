...
class AugmentDFS2:
    ...
    def DFS(self,residual,parentPath,wait,visited,sequence):
        # loop
        if not wait:
            wait.append(self.s)
        if not sequence:
    ...
    def maxflow(self):
        ...
        while True:
            ...
            flag, parentPath,wait, visited,sequence = self.DFS(residual,parentPath,wait,visited,sequence)
            count = count + 1
            if not flag:
                visited = [False] * (self.t+1)
                parentPath = -1*np.int32(np.ones(self.t+1))
            ...
            seq_delete = sequence[sequence.index(bottleneck_label):]
            for node in seq_delete:
                ...
                try:
                    ...
                    pass
            sequence = sequence[:sequence.index(bottleneck_label)]
